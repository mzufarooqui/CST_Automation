function f_SendMail(sender,senderPassword,recipients, subject, mail_message, attachments)

% f_SendMail sends a mail message through Gmail
% function f_SendMail(subject, recipients, mail_message, attachments)
% input arguments:
...
%                      Use [] for none.
% Define these:
mail = sender'; %Your GMail email address
password = senderPassword; %Your GMail password
[senderEmailAddressOnly, domainName] = strtok(sender, '@');
SMTP_Server = ['smtp.' domainName(2:end)];
% Don't touch unless you need to change the Email supplier (currently Gmail)
setpref('Internet','E_mail',mail);
% setpref('Internet','SMTP_Server','smtp.gmail.com');
% setpref('Internet','SMTP_Server','smtp.polito.it');
