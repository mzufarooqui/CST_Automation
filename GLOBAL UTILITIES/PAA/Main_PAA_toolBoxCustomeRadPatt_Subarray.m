...
replarray   = phased.ReplicatedSubarray('Subarray',subULA,'GridSize',[1 Nsubarray],'SubarraySteering','none');
...
figure,
subplot(2,1,1)
 pattern(replarray,fc,-180:1:180,0,'Type','directivity','CoordinateSystem','rectangular','PropagationSpeed',c,'Weights',w);
 xlabel('\theta [deg]');ylabel('Directivity [dB]');title(sprintf('f = %.1fGHz',fc/1e9))
...
subplot(2,1,2)
 pattern(replarray,fc,-180:1:180,0,'Type','directivity','CoordinateSystem','rectangular','PropagationSpeed',c,'Weights',w,'SteerAngle',steerang);
 xlabel('\theta [deg]');ylabel('Directivity [dB]');title(sprintf('f = %.1fGHz, with PS before each',fc/1e9))
...
% replarray = phased.ReplicatedSubarray('Subarray',subURA,'Layout','Rectangular','GridSize',[N N/Nsubarray],'SubarraySteering','None');
% figure,viewArray(replarray)
% replarray.getNumElements
% 
