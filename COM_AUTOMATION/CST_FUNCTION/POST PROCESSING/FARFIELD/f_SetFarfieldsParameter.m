function f_SetFarfieldsParameter(oMWS,oFarField)
...
%  oFarField.origin
%       The origin type of the farfield calculation. originType can have one of the following values:
...
%  oFarField.userOrigin (double x, double y, double z )
%           Sets origin of the farfield calculation if the origin type is set to free.Please note: x, y, and z must be double values here. Any expression is not allowed.
...
if isfield(oFarField,'vary'),            invoke(oFarfieldPlot,'Vary',oFarField.vary);                     end
...
if isfield(oFarField,'origin'),          invoke(oFarfieldPlot,'Origin',oFarField.origin);                 end
if isfield(oFarField,'setFrequency'),    invoke(oFarfieldPlot,'SetFrequency',oFarField.setFrequency);     end
if isfield(oFarField,'userOrigin'),      invoke(oFarfieldPlot,'Userorigin',oFarField.userOrigin(3),oFarField.userOrigin(2),oFarField.userOrigin(3));end     
