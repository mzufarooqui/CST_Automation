function scriptName = f_CST_ModFile_To_Matlab_Script(LIB_PATH,cstProjectPath,cstProjectName)

scriptName       = ['Main_CST_Proj_Recovery_' strtok(cstProjectName,'.') '.m'];
fModel3D         = fopen([cstProjectPath '\Model\3D\Model.mod'],'rt+');
...
% paramList        = f_GetAllParametersFromModel_par_File([cstProjectPath '\Model\3D\Model.par']);
[ paramList numParam ] = f_GetAllParametersFromModel_par_File([cstProjectPath '\Model\3D\Model.par']);
...
for paLiSaInd = 1:size(paramName,1)
    ...
    fprintf(fStorParamFunc,'f_StoreParameterWithDescription(oMWS,''%s'',%s,''%s'');\n',paramList{paLiSaInd,1},paramList{paLiSaInd,1},paramList{paLiSaInd,3});
        ...
        fprintf(fStorParamFunc,'f_StoreParameter(oMWS,''%s'',%s);\n',paramList{paLiSaInd,1},paramList{paLiSaInd,1});
    end
end
