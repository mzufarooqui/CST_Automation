...
% %%%% inizio vecchia parte di codice per imposizione segno dei modi
% xvet=PatchInfo(1).x;
% yvet=PatchInfo(1).y;
...
% end
% %%%% fine vecchia parte di codice per imposizione segno dei modi
% 

%%%% inizio nuova parte di codice per imposizione segno dei modi
for k=1:NumPatch
    xvet=PatchInfo(k).x;
    yvet=PatchInfo(k).y;
    Weight=PatchInfo(k).Weight;
    fEspan=PatchInfo(k).fEspan;
    coeff_mn=PatchInfo(k).coeff_mn; 

    fRef=ones(size(xvet))+xvet+yvet;
    if(k==1)
        IntRef=conj(fRef.*Weight)*(coeff_mn.'*fEspan).';
    else
        IntRef=IntRef+conj(fRef.*Weight)*(coeff_mn.'*fEspan).';
    end
end
SignVett=sign(IntRef);
for k=1:NumPatch
    PatchInfo(k).coeff_mn=PatchInfo(k).coeff_mn.*(ones(size(PatchInfo(k).coeff_mn,1),1)*SignVett); 
end
%%%% fine nuova parte di codice per imposizione segno dei modi
